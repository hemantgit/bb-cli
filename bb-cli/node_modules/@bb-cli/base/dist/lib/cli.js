...
var Command = _commander2.default.Command;
/**
...
function humanReadableArgName(arg) {
  var isVariadic = arg.variadic === true ? '...' : '';
  var tplOutput = '' + arg.name + isVariadic;
...
Command.prototype.version = function version(str) {
  var flags = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : '-v, --version';
...
Command.prototype.helpInformation = function helpInformation() {
  var headingOptions = _ui2.default.colors.heading('OPTIONS: ');
  var headingDescription = _ui2.default.colors.heading('NAME: ');
  var headingUsage = _ui2.default.colors.heading('SYNOPSIS: ');
  var desc = this._description ? [headingDescription, this.name() + ' - ' + this._description, ''] : [];

  var cmdHelp = this.commandHelp();
  var optHelp = this.optionHelp();
  var manHelp = this._man;
  var cmds = cmdHelp || [];
  var opts = optHelp ? optHelp.replace(/^/gm, '  ') : [];
  var cmdName = this._name;
  if (this._alias) {
    cmdName = cmdName + ' | ' + this._alias;
  }
  var usage = [headingUsage, cmdName + ' ' + this.usage(), ''];
  var options = [headingOptions, opts];
  return desc.concat(usage).concat(cmds).concat(options).concat(manHelp).join('\n');
};
/**
 * @override
 * @public
...
Command.prototype.commandHelp = function commandHelp() {
  ...
  var headingTitle = _ui2.default.colors.heading('COMMANDS:');
  var commands = this.commands.filter(function (cmd) {
    ...
    var alias = cmd._alias ? '| ${cmd._alias}' : '';
    var options = cmd.options.length ? '[options]' : '';
    var tplCmd = cmd._name + ' ' + alias + ' ' + args + ' ' + options;
    return [tplCmd, cmd._description];
  });
  var width = commands.reduce(function (max, command) {
    return Math.max(max, command[0].length);
  }, 0);
  var addPadding = function addPadding(cmd) {
    return pad(cmd[0], width) + (cmd[1] ? cmd[1] : '');
  };
...
Command.prototype.man = function man(filePath) {
  var manContent = '';
  try {
    manContent = _utils2.default.md2term(_fs2.default.readFileSync(filePath, 'utf8').toString());
    _log2.default.verbose('MAN', '' + filePath);
  } catch (err) {
